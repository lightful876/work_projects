...
#Ecommerce Report generation for Operational Reporting on eCommerce Platform
...
import numpy as np
import os
os.chdir('C:\\Users\\roy_shaw\\Desktop\\Work_Files\\Weekly')
...
# The values in the data frame df_pur now contain only values for completed orders purchased within the pre-determined timeframe.
...
# The below code combines the remainder of the purchases which were completed as detailed above   
df_main = pd.concat([df_pur, df_comp])

# Removing duplicated values from the main data frame, for correctness.
...
# The removal of fields that are based on business practices.
# We do this for both the main and empty dataframes because we want to ensure that these types of purchases are not processed because of business practices.
...
# Repeat of to_datetime for certainty in code.
...
# Conditioning the data by dropping all NaN values that were present in the main dataframe's "Completed At" column indicating that the order was not completed and were cancelled.
...
# Filtering the transactions which were approved by the payment gateway. Leaving the transactions which were declined or otherwise.
...
# This block of code is based on a specification in the records that would exclude certain transaction based on business operation (funds submitted for higher purchase payments).
# Jamaica
...
# Updating for values of Barbados that are remaining after removing the Jamaica data
# Barbados
...
# Updating for values of Guyana that are remaining after removing the Jamaica and Barbados data
# Guyana
...
# Updating for values of OECS that are remaining after removing the Jamaica, Barbados, and Guyana data
# OECS
...
# Updating for values of Belize that are remaining after removing the Jamaica, Barbados, Guyana and OECS data
# Belize
...
for i in range(len(lst_GT)):
    if (lst_GT[i] == lst_ST[i]) & (lst_GT[i]%5 == 0) & (lst_GT[i] <= 750):
        ...
        lst_validity.append('True')
df_validation['is_RFpymt'] = lst_validity
df5.drop(new_df5.index[new_df5['Purchase Point'] == 'Belize'], axis = 0, inplace = True)
count_RF = count_RF.append(df_validation[df_validation['is_RFpymt'] == 'False'])
df_validation.drop(df_validation.index[df_validation['is_RFpymt'] == 'False'],axis = 0, inplace = True)
